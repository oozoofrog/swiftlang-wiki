...
홈 페이지를
처음 방문한 독자에게 더 가벼운 시작점으로 다듬고,
반복 방문자와 구현 중심 독자를 위한 별도 압축 인덱스가 추가됐다.
...
핵심 방향:
- home은 시작 경로와 대표 허브 중심으로 slim down
- 반복 방문자용 긴 링크 묶음은 power-user page로 분리
- 같은 세부 문서를 공유하되, 진입 경험만 분리
